...
//----------------------------------------------------------------------
//advanced functionality 1.1: number of questions answered correctly

app.get('/getCorrectAnswers/:port_id', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
         ...
         var querystring = " select array_to_json (array_agg(c)) from ";
          querystring += "(SELECT COUNT(*) AS num_questions from public.quizanswers where (answer_selected = correct_answer) and port_id = $1) c;";
...
//---------------------------------------------------------------
//functionality 1.2: give ranking
app.get('/getRanking/:port_id', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
          ...
          //colnames = colnames + "answer_2, answer_3, answer_4, port_id, correct_answer";
          var param1 =  req.body.port_id ;
          var param2 =  req.body.answer_selected;
          var param3 =  req.body.correct_answer;
         ...
         var querystring = " select array_to_json (array_agg(hh)) from ";
          querystring += "(select c.rank from (SELECT b.port_id, rank()over (order by num_questions desc) as rank ";
          querystring += "from (select COUNT(*) AS num_questions, port_id from public.quizanswers ";
          querystring += "where answer_selected = correct_answer group by port_id) b) c where c.port_id = $1) hh;";
          console.log(querystring);
          var port_id = req.params.port_id; //
          // run the second query
          client.query(querystring,[port_id],function(err,result){
...
app.get('/getTopFive', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
          ...
          // note that query needs to be a single string with no line breaks so built it up bit by bit
          var querystring = "select array_to_json (array_agg(c)) from ";
          querystring += "(select rank() over (order by num_questions desc) as rank , port_id ";
          querystring += "from (select COUNT(*) AS num_questions, port_id ";
          querystring += "from public.quizanswers ";
          querystring += "where answer_selected = correct_answer ";
          querystring += "group by port_id) b limit 5) c; ";
          console.log(querystring);
...
app.get('/dailyParticipation_oneUser/:port_id', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
         ...
         var querystring = "select array_to_json (array_agg(c)) ";
          querystring += "from  ";
          querystring += "(select * from public.participation_rates where port_id = $1) c";
          console.log(querystring);
          var port_id = req.params.port_id; //
          // run the second query
          client.query(querystring,[port_id],function(err,result){
...
app.get('/dailyParticipation_allUsers', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
          ...
          // note that query needs to be a single string with no line breaks so built it up bit by bit
          var querystring = "select  array_to_json (array_agg(c)) from  ";
          querystring += "(select day, sum(questions_answered) as questions_answered, sum(questions_correct) as questions_correct ";
          querystring += "from public.participation_rates  ";
...
//--------------------------------------
//2.3 all questions added in the last week
app.get('/getAllQuestions', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
          ...
          var querystring = "SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features FROM ";
          querystring += "(SELECT 'Feature' As type     , ST_AsGeoJSON(lg.location)::json As geometry, ";
          querystring += "row_to_json((SELECT l FROM (SELECT id, question_title, question_text, answer_1, answer_2, answer_3, answer_4, port_id, correct_answer) As l  ";
...
app.get('/FiveMostDifficultQuestions', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
          ...
          querystring += "(select c.* from public.quizquestion c inner join ";
          querystring += "(select count(*) as incorrectanswers, question_id from public.quizanswers where ";
          querystring += "answer_selected <> correct_answer group by question_id ";
          querystring += "order by incorrectanswers desc limit 5) b on b.question_id = c.id) d; ";
          console.log(querystring);
          // run the second query
          client.query(querystring,function(err,result){
            //call `done()` to release the client back to the pool
...
//---------------------------------------
//2.6 -- Quiz App: map showing the last 5 questions that the user answered 
//(colour coded depending on whether they were right/wrong the first time they answered the question)
//-- Return result as GeoJSON
//-- $1 is the port id

app.get('/lastFiveQs/:port_id', function (req,res) {
     pool.connect(function(err,client,done) {
        if(err){
            console.log("not able to get connection "+ err);
            res.status(400).send(err);
          ...
          var param1 =  req.body.port_id ;
          var param2 =  req.body.question_title
          var param3 =  req.body.question_text
          var param4 =  req.body.answer_1
          var param5 =  req.body.answer_2
          var param6 =  req.body.answer_3
          var param7 =  req.body.answer_4
          var param8 =  req.body.correct_answer;
          var param9 =  req.body.answer_selected;
          // now use the inbuilt geoJSON functionality
          // and create the required geoJSON format using a query adapted from here:
          // http://www.postgresonline.com/journal/archives/267-Creating-GeoJSON-Feature-Collections-with-JSON-and-PostGIS-functions.html, accessed 4th January 2018
          // note that query needs to be a single string with no line breaks so built it up bit by bit
          var querystring = "SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features  FROM ";
          querystring += "(SELECT 'Feature' As type     , ST_AsGeoJSON(lg.location)::json As geometry, ";
          querystring += "row_to_json((SELECT l FROM ";          
          querystring += "  (SELECT id, question_title, question_text, answer_1, answer_2, answer_3, answer_4, port_id, correct_answer, answer_correct) As l ";
          querystring += " )) As properties FROM ";
          querystring += "(select a.*, b.answer_correct from public.quizquestion a ";
          ...
          querystring += "where port_id = $1 order by timestamp desc limit 5) b ";
          querystring += "on a.id = b.question_id) as lg) As f ";
          console.log(querystring);
          var port_id = req.params.port_id; //
          // run the second query
          client.query(querystring,[port_id],function(err,result){
